...
//
// Retuned for Cyphernaut: the stock recipe is a bright green mesh, which would
// bury a near-black page. The palette below walks the site's own hue from
// --canvas up to a dim olive, so the accent registers as a haze rather than a
// colour field. See the note on `colors` for why the top stop is not #BFFF00.
// Grain is 0 because the page already lays its own film grain over everything
// at z-index 90; two grain passes stack into visible noise.
...
const UNIFORMS = {
  // Cyphernaut ramp: --canvas #070908 -> --canvas-lit #0B120A ->
  // a dark olive bridge #1E2A08 -> --accent #BFFF00. Slots 4-7 repeat the
  // canvas so any stray sample falls back to page black rather than lime.
  // These are deliberately near-black. Under `screen` the canvas value is
  // added to the page, so the top stop is what the field's brightest point
  // becomes — a literal #BFFF00 there washes the whole viewport lime and
  // destroys body-copy contrast. The accent is carried as a dim olive
  // (~#1F2900) so it reads as a haze in the page's own hue, not a colour field.
  colors: [
    [0.02353, 0.03137, 0.02353],
    [0.04706, 0.06667, 0.03529],
    [0.10980, 0.15294, 0.02745],
    [0.24314, 0.32157, 0.01961],
    [0.02353, 0.03137, 0.02353],
    [0.02353, 0.03137, 0.02353],
    [0.02353, 0.03137, 0.02353],
    [0.02353, 0.03137, 0.02353],
  ...
  saturation: 1.0,
  hue: 0.0,
  // Pulls the corners back to page black so the field concentrates centrally
  // instead of lifting the whole viewport.
  vignette: 0.32,
  blur: 0.0,
  // 0, not 0.091: globals.css already paints film grain over the whole page
  // at z-index 90. Two grain passes stack into visible noise.
  grain: 0.0,
  seed: 1453.0,
  rotate: 0.0,
...
export function ShaderBackground({ className }: { className?: string }) {
  ...
  useEffect(() => {
    ...
    // A drifting field is exactly the kind of ambient motion reduced-motion
    // asks to stop. Render one static frame instead of dropping the layer,
    // so the composition still reads the same.
    ...
    // Arrow consts, not `function` declarations: hoisted declarations lose the
    // non-null narrowing on `canvas` and `gl` under TS strict, because TS must
    // assume they could be invoked before the guards above ran.
    const requestRender = () => {
    ...
    const render = (now: number) => {
      ...
      if (minFrameMs && now - lastDrawn < minFrameMs) {
        requestRender()
        return
      }
      lastDrawn = now
